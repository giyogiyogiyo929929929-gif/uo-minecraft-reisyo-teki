...
import { getDistrictAdjacencyYields, getDistrictFlatYields, getDistrictPopulationYields, getDistrictBuildingFlatYields, tickDistrictConstruction, isSacredSiteTile } from "./districts.js";
...
const YIELD_KEYS = ["food", "production", "oil", "faith", "iron"];
...
export function getCityCurrentYields(cityKey, tiles) {
    ...
    const totals={food,production,oil,faith,iron};
    for(const src of sources)for(const key of YIELD_KEYS)totals[key]+=src[key]??0;
    const result={food:totals.food*YIELD_MULTIPLIER,production:Math.max(1,totals.production)*YIELD_MULTIPLIER,oil:totals.oil*YIELD_MULTIPLIER,faith:totals.faith*YIELD_MULTIPLIER,iron:totals.iron*YIELD_MULTIPLIER};cityYieldCache.set(cityKey,result);return result;
}
...
function processPlayerTurnStart(playerId){const config=getMapConfig();if(!config)return;const tiles=getTiles();if(checkAndAnnounceVictory(tiles))return;const playerCities=[];let movementRefreshed=false;for(const key in tiles){const t=tiles[key],unit=t.combatUnit;if(unit?.ownerId===playerId){unit.movementRemaining=unit.movement??0;movementRefreshed=true;}const religiousUnit=t.religiousUnit;if(religiousUnit?.ownerId===playerId){religiousUnit.movementRemaining=religiousUnit.movement??0;religiousUnit.hasProselytizedThisTurn=false;movementRefreshed=true;}if(t.ownerId===playerId&&t.city)playerCities.push({key,tile:t});}if(!playerCities.length){if(movementRefreshed)setTiles(tiles);return;}
    ...
    for(const c of playerCities){const y=getCityCurrentYields(c.key,tiles);cityFoodIncomes[c.key]=y.food;cityProductionIncomes[c.key]=y.production;cityFaithIncomes[c.key]=y.faith??0;for(const r of STRATEGIC_RESOURCES)strategicIncomes[r.key]+=y[r.key]??0;c.tile.city.currentTurnProduction=y.production;}
    ...
    setTiles(tiles);if(checkAndAnnounceVictory(tiles))return;const player=getCivStorageHandle(playerId);if(player){if(hasFoundedReligion(player))for(const key in tiles){const t=tiles[key];if(!isSacredSiteTile(t,playerId))continue;const cityTile=t.belongsToCityKey?tiles[t.belongsToCityKey]:null;if(cityTile?.city)applySacredSitePressure(cityTile.city,playerId);}let totalPop=0;for(const c of playerCities)totalPop+=c.tile.city.population;const tr=grantProgressPoints(player,"technology",totalPop),cr=grantProgressPoints(player,"civic",totalPop);if(tr)summaryReport.unshift(tr);if(cr)summaryReport.unshift(cr);for(const r of STRATEGIC_RESOURCES){const income=strategicIncomes[r.key];if(income>0){const stock=(player.getDynamicProperty(r.prop)??0)+income;player.setDynamicProperty(r.prop,stock);summaryReport.unshift(`${r.prefix}${r.label}収入: +${income} 個を獲得！ (現在の在庫: ${stock} 個)`);}}player.sendMessage("§6=== 💡 都市のターン報告 ===");summaryReport.forEach(m=>player.sendMessage(m));player.sendMessage("§6========================");}}
