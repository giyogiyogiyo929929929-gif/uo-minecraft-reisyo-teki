...
//
// 【全体の流れ】
// 1. 聖地(district)を建設 → 社(district building)を建てると、伝道者を信仇力で購入可能になる。
// 2. 国家全体(全都市合計)の信仇力が100に達し、かつ聖地を1つ以上持っていれば、宗教を創始できる
//    (!civ foundreligion)。1国家につき宗教は1つまで。100信仇力は消費されない(閾値の確認のみ)。
// 3. 聖地のある都市は、自国が宗教を創始していれば、毎ターン自国の宗教の宗教的圧力を+100する。
// 4. 伝道者は隣接する都市に布教でき、布教力を1消費して宗教的圧力を加える。
// 5. 都市の人口 × (その宗教の圧力 / 都市の累積圧力) が、その宗教の信仇者数。
//    信仇者数が人口の過半数を超えた宗教が、その都市の主流宗教になる。
...
const RELIGION_FOUND_THRESHOLD = 100; // 国家全体の信仇力合計がこの値に達すると宗教を創始できる
...
/**
 * 宗教ユニットの定義(信仇力で購入する、戦闘ユニットとは別レイヤーのユニット)。
 * @typedef {Object} ReligiousUnitDef
 * @property {string} label 表示名
 * @property {string} icon 表示アイコン
 * @property {number} cost 購入に必要な信仇力(その都市の貯留信仇力から即座に消費される)
...
/** 国家全体(全都市合計)の信仇力を合計する。playerCitiesは [{key, tile}] 形式。 */
 ...
 * ・国家全体の信仇力が閾値(100)に達していること(消費はされない)。
 */
export function canFoundReligion(civHandle, civId, playerCities, tiles) {
    ...
    if (totalFaith < RELIGION_FOUND_THRESHOLD) {
        return { ok: false, message: `§c宗教の創始には国家全体で信仇力${RELIGION_FOUND_THRESHOLD}が必要です。(現在: ${Math.floor(totalFaith)})` };
    }
    return { ok: true };
}
...
/** 都市の主流宗教(信仇者数が人口の過半数を超える宗教のID=国家ID)を判定する。無ければnull。 */
