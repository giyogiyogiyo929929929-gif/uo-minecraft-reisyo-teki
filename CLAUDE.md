...
- `civ:matchSettings` — `{ yieldMultiplier, diplomacyEnabled, botTurnDelayTicks, peaceEnabled, logsEnabled }` (`state.js`'s `getMatchSettings`/`setMatchSettings`), OP-editable match rules. Unlike the keys above, **this one is intentionally NOT cleared by `resetAll()`** — it's meant to survive a game reset so an OP doesn't have to re-configure it for every test match. `state.js` also exports `broadcast(message)`, a `world.sendMessage()` wrapper gated on `logsEnabled` — nearly every action-log broadcast across the codebase goes through it (`commands.js`, `bots.js`, `ui.js`, `main.js`, and the non-victory announcements in `turns.js`) so an OP can silence chat spam during unattended all-Bot matches. Victory announcements (solo/religious/alliance, and the debug instant-victory) call `world.sendMessage()` directly in `turns.js` and are intentionally exempt — they always show regardless of `logsEnabled`.
